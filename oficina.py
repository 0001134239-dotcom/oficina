...
if st.session_state.logado and st.session_state.role in ["admin", "superadmin"]:
    with tab2:
        ...
        with st.form("cadastro"):
            ...
            armario = st.number_input("Armário", min_value = 1, step = 1)
            prateleira = st.number_input("Prateleira", min_value=1, step=1)
...
if st.session_state.logado and st.session_state.role in ["admin", "superadmin"]:
    with tab3:
        if st.session_state.role == "superadmin":
            ...
            role_series = usuarios_df.loc[usuarios_df["usuario"] == target, "role"]
